/*
 * BatchJ: https://github.com/vcaldas/BatchJ
 *
...
public class Utils implements PlugIn {
	...
	static final String DOC_URL = "http://imagej.net/BatchJ";

	/** The URL to BAR's GitHub repository */
	static final String SRC_URL = "https://github.com/vcaldas/BatchJ";

	/** The URL to BAR's GitHub repository */
	static final String API_URL = "http://vcaldas.github.io/BatchJ/apidocs/";

	/** The absolute path to the /BAR directory */
	static final String BATCHJ_DIR = IJ.getDirectory("plugins")
			+ "Scripts" + File.separator + "BatchJ" + File.separator;

	/** The absolute path to the /BAR/Snippets/ directory */
	static final String SNIPPETS_DIR = BATCHJ_DIR +"Snippets" + File.separator;

	/** The absolute path to the /BAR/lib/ directory */
	static final String LIB_DIR = BATCHJ_DIR + "lib" + File.separator;
	...
	private void moveSubmenu(final String subMenu) {
		...
		if (popmenuPos==null || barmenuPos==null) {
			IJ.error("BatchJ v"+ VERSION +" Error", "Some menu items are not accessible.\n"
		...
		if (popmenuPos==-1) { // parent is MenuBar
			...
			if (!IJ.macroRunning())
				IJ.showMessage("BatchJ v"+ VERSION, ""+ subMenu +"> transferred to context menu.\n"
			...
			if (!IJ.macroRunning())
				IJ.showStatus("BatchJ>" + subMenu + "> transferred to main menu.");
	...
	/**
	 * Opens the specified file in the "Snippets" directory of BatchJ. No tests
	...
	/**
	 * Opens the specified file in the "lib" directory of BatchJ. No tests
	 * assessing the existence of the specified file are performed.
	 *
	 * @param filename
	...
	public static String getAnalysisDir() {
		return BATCHJ_DIR + "Analysis" + File.separator;
	}
	...
	public static String getAnnotationDir() {
		return BATCHJ_DIR + "Annotation" + File.separator;
	}
	...
	public static String getDataAnalysisDir() {
		return BATCHJ_DIR + "Data_Analysis" + File.separator;
	}
	...
	public static String getSegmentationDir() {
		return BATCHJ_DIR + "Segmentation" + File.separator;
	}
		...
		if (gd.wasCanceled()) {
			...
			// Clipboard
				...
				if (clipboard==null || clipboard.isEmpty()) {
					...
					try {
						final File temp = File.createTempFile("BatchJclipboard", ".txt");
						temp.deleteOnExit();
